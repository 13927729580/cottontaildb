...
inline class FloatVectorValue(val data: FloatArray) : RealVectorValue<Float> {
     ...
     */
    override fun get(start: Int, length: Int) = FloatVectorValue(
            FloatArray(length) {
                data[start + it]
            }
    )
    ...
    override fun minus(other: VectorValue<*>, start: Int, otherStart: Int, length: Int) = when (other) {
        ...
        is DoubleVectorValue -> FloatVectorValue(FloatArray(length) {
            (this.data[start + it] - other.data[otherStart + it]).toFloat()
        })
